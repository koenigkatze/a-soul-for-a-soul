...
//	private CustomMapProperties mapProperties;
...
//		sound.setVolume(id, 0.25f);
	...
	{
		Messages.publish(LevelMessageCodes.LEVEL_DATA_LOADING, "island_3");
	...
	private void loadMapData(final MapLoader mapLoader)
	{
		// mapProperties = mapLoader.loadMapProperties();
		mapLoader.loadBlockedMapParts(bodyFactory);
		mapLoader.loadCharacterEntitiesFromMap();
		mapLoader.loadPassiveObjects();
		mapLoader.loadActiveMapObjects();
	}
	...
	private void loadLevelSystems()
	{
		...
		final TiledMapRenderer tiledMapRenderer = new OrthogonalTiledMapRenderer(mapLoader.getMap());
